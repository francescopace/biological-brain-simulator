...
def audit(brain, images, *, train_steps, rest_steps, per_neuron=False, learning_rule="pair"):
    ...
    for number, image in enumerate(images):
        ...
        for _ in range(train_steps):
            ...
            ltp = after_ltp.double() - before.double()
            ltd = after_ltd.double() - after_ltp.double()
            row["ltp_l1"] += float(ltp.abs().sum())
            row["ltd_l1"] += float(ltd.abs().sum())
            row["net_step_l1"] += float((after_ltd.double() - before.double()).abs().sum())
        ...
        row["stdp_signed_sum"] = float((after_stdp.double() - initial.double()).sum())
