...
An [isolated conductance-LIF reference pilot](BENCHMARKS.md#isolated-conductance-lif-reference-pilot) tests the triplet-STDP variant in Diehl and Cook's released code. Its completed paired experiment found a consistent STDP benefit in spike-based classification across three seeds. A subsequent [train-only decoder selection](BENCHMARKS.md#train-only-spikevoltage-decoder-selection) improved the combined spike/voltage readout without retraining the network. A [frozen evaluation on new images](BENCHMARKS.md#frozen-decoder-evaluation-on-fresh-mnist-images) confirmed the decoder gain, while finding only a small, uncertain advantage over voltage-only decoding and the initial network. Separate [frozen-inference](BENCHMARKS.md#frozen-inference-timestep-sensitivity) and [early-training](BENCHMARKS.md#early-training-timestep-sensitivity) diagnostics replay matched input events across timestep grids; [weight/theta swaps](BENCHMARKS.md#frozen-weight-and-threshold-swaps) probe their separate effects without retraining. The linked reports separate these effects from the numerical limits. This experimental implementation leaves Iris and the production simulator unchanged and does not reproduce the paper's 87% power-law result.

A [deterministic timing audit](BENCHMARKS.md#deterministic-spike-timing-and-refractory-audit) identified late-inhibition effects and a decimal-grid refractory defect. The [corrected reference v2](BENCHMARKS.md#corrected-reference-dynamics-v2) uses integer refractory deadlines and eight complete internal steps per 0.5 ms input interval. Existing checkpoints retain v1 dynamics. A [matched 50-image training check](BENCHMARKS.md#matched-v1v2-early-training-check) found that halving v2's step still changes learned weights and neuron-level responses substantially; v2 used about 3.19 times v1's process CPU time in that short paired comparison. The reported MNIST scores, including 69.27% on fresh validation images, belong to v1. V2 accuracy has not yet been measured.
